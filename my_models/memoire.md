...
| Five bug fixes pending upstream | Five SMIA framework files are patched at Docker image build time: `smia_agent.py` (Patch 1), `acl_handling_behaviour.py` (Patch 2), `operator_gui_logic.py` (Patch 3), `negotiating_behaviour.py` (Patch 4), `handle_negotiation_behaviour.py` (Patch 5). If the upstream image is updated, each patch file may need review for conflicts. The long-term fix is upstream PRs to the SMIA repository. Full documentation in `my_models/PATCHES.md` |
...
- Submit all five patch files as upstream PRs to the SMIA repository to eliminate the build-time overrides. Remove `TODO BORRAR BUG TEST` warning log lines from `acl_handling_behaviour.py` and `handle_negotiation_behaviour.py` before PR submission.
...
| Operator GUI crash on Load: `SyntaxError: Unexpected non-whitespace character after JSON at position 4` | Two latent bugs in `operator_gui_logic.py`'s `hasParameter` processing block, never triggered because no default SMIA AASX uses `hasParameter` relationships. Bug 1: `css_elems_info['skillData']` — key never exists → `KeyError`. Bug 2: `param_set.add(skill_param)` where `skill_param` is a list → `TypeError: unhashable type: 'list'`. Both exposed when `SkillParameter_color` + `hasParameter` rel were added to the orchestrator AASX. | Patched `operator_gui_logic.py`: replaced the entire block with `self.myagent.skills_info[skill].update(p.id_short for p in skill_params_list)` — checks the correct dict, iterates the list, and stores `id_short` strings |
| Concurrent negotiations produce incorrect outcomes (wrong machine wins or negotiation corrupted) | `NegotiatingBehaviour` registered `HandleNegotiationBehaviour` without a per-thread SPADE template. SPADE delivered every PROPOSE to every registered handler regardless of negotiation thread — concurrent negotiations contaminated each other. | Patched `negotiating_behaviour.py`: pass a combined per-thread template (`PROPOSE|REQUEST` filtered by `msg.thread`) to `add_behaviour()` |
| Negotiation deadlock with 3+ eligible machines; orchestrator waits indefinitely for winner INFORM | Three interacting problems: (A) initial PROPOSE silently dropped if peer hasn't registered its `HandleNegotiationBehaviour` yet, no retry; (B) machine exits behaviour immediately on loss — late PROPOSE hits dead handler; (C) 10-second blocking receive prevents retry logic | Patched `handle_negotiation_behaviour.py`: short 0.01 s receive timeout + iteration budget (`max(5, n+3)`); deferred exit (keep behaviour alive until budget exhausted); REQUEST-for-negValue retry at random iterations 20–60% of budget; individual PROPOSE dispatch with micro-delays; thread reservation/release via `add_reserved_thread`/`remove_reserved_thread` |
...
| **C — Bug fix** | SMIA framework files with targeted bug corrections; all other framework code is unmodified | `smia_agent.py` (Patch 1), `acl_handling_behaviour.py` (Patch 2), `operator_gui_logic.py` (Patch 3), `negotiating_behaviour.py` (Patch 4), `handle_negotiation_behaviour.py` (Patch 5) — all pending upstream PRs |
...
1. **Node-RED virtual factory dashboard (MVP)**: The current Node-RED implementation bridges HTTP to MQTT commands sent to a physical fischertechnik crane. The next step is to replace the MQTT output with a Node-RED Dashboard (using the `node-red-dashboard` package) that renders a visual warehouse simulation: four coloured warehouse slots, animated picking state per machine, real-time busy/free status panel, and operations log. The HTTP API endpoints and per-machine busy flag logic remain unchanged — only the final output changes from a physical command to a dashboard visualisation. This makes the entire system self-contained and demonstrable from any laptop with Docker installed, enabling the SMIA authors to run and validate the full Case 1 scenario without physical hardware. The dashboard is accessible at `http://localhost:1880/ui`.

2. **Case 1 extensions**: Case 1 (multi-agent orchestration with six machine SMIAs and an orchestrator) has been implemented and validated (§16). Future improvements include: strict busy-rejection (returning FAILURE to the operator when all machines are busy rather than queueing the task); configurable colour-to-slot mapping in the AASX (currently hardcoded in `orchestrator_dispatch_behaviour.py`); and upstream contribution of the FIPA-CNP initiator behaviour and five framework bug fixes to the SMIA repository.
